...
stimfilespec    = PARAMS.stimefilespec;
datafilespec    = PARAMS.datafilespec;
dBin            = PARAMS.binsize;

%Load stim file and data file
%Deviates from old fucntion by using excel input
[stimtime, stimgroup, S] = parse_excel(stimfilespec, 0);
[datatime, datagroup, S] = parse_excel(datafilespec, 0);
clear S
time = datatime;
...
nStim = size(data, 2); %returns vector of # data columns
for n = 1:nStims
...
%data spike times
data = datagroup{:};
[spiketimes, spikelocs, peaktimes, peaklocs] = detectspikes(time, data, signaltype, threshtype, thresh, peakflat, displayflag);
