...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  /* The input must be a noncomplex spcalar double.*/
  arows = mxGetM(prhs[0]);
  acols = mxGetN(prhs[0]);
  if (!mxIsDouble(prhs[0]) || mxIsComplex(prhs[0]) || !(arows == 1 && acols == 1)) 
  {
    mexErrMsgTxt("Input argument 1 must be a noncomplex scalar double. See Help.");
  }
  brows = mxGetM(prhs[1]);
  bcols = mxGetN(prhs[1]);
  if (!mxIsDouble(prhs[1]) || mxIsComplex(prhs[1]) || !(brows == 1 && bcols == 1)) 
  {
    mexErrMsgTxt("Input argument 2 must be a noncomplex scalar double. See Help.");
  }
  rhorows = mxGetM(prhs[2]);
  rhocols = mxGetN(prhs[2]);
  if (!mxIsDouble(prhs[2]) || mxIsComplex(prhs[2]) || !(rhorows == 1 && rhocols == 1)) 
  {
