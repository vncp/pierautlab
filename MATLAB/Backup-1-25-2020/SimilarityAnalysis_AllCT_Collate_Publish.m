...
%ANCOVA separate lines (home-made). Ctrl: c = 1 ; Other: c = 2
    ...
    [EST_R_gzn{b}, pF_R(b), Fstat_R(b)] = PolFitandFtest (Npol, StimPairAll_R{2}{b}, OutPairAll_R{2}{b}, StimPairAll_R{1}{b}, OutPairAll_R{1}{b}, 1);
    ...
    GroupBlock_a = cat(2, EE_matrix, SH_matrix);
    %[Hanc{b},TABanc{b},COEFanc{b},Sanc{b}] = aoctool(PoolX, PoolY, GroupBlock_a, 0.05,'Sin','Sout','Protocol','off','separate lines');
    %COMPancS{b} = multcompare(Sanc{b}, 'Alpha', .05, 'Estimate', 'slope', 'display','on');
    %COMPancInt{b} = multcompare(Sanc{b}, 'Alpha', .05, 'Estimate', 'intercept', 'display','on');
    %[MAT_pvalS{b}, MAT4_pS{b} ] = pvalmat(COMPancS{b});
    %[MAT_pvalI{b}, MAT4_pI{b} ] = pvalmat(COMPancInt{b});
    [ Tanc, panc, FANCOVAN, pANCOVAN, stats ] = mancovan(PoolY, GroupBlock_a, PoolX)
    disp(panc)
    ...
    subplot(3,length(tau),b) 
        ...
        ylabel('pw Sout')
        set(gca, 'xlim', [0 1], 'ylim', [0 1])
        title(['R ' num2str(Xbinsize(okb)) 'ms, p = ' num2str(pF_R(b)), ', F = ' num2str(Fstat_R(b))])
