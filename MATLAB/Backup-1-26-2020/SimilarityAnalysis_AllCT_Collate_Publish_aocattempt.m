...
% PatSep 
load('C:\Users\vince\lab\pierautlab\OriginalTest\data\SimilarityAnalysis_SH_NaN.mat');
Collate{1} = Cdata; % SH
Valid{1} = [1:4]; %[6, 10, 11]; %[4,5,7,8,9] %
clearvars -except Collate Valid
load('C:\Users\vince\lab\pierautlab\OriginalTest\data\SimilarityAnalysis_EE_NaN.mat');
Collate{2} = Cdata; % EE
Valid{2} = [1:4];
clearvars -except Collate Valid
...
%Debug        PSRbuc{c}{b} = signrank(PatSepBuC{c}{b});
%Debug        PSRbuo{c}{b} = signrank(PatSepBuO{c}{b});
%Debug        PSRfr{c}{b} = signrank(PatSepFR{c}{b});
...
ColMapSF = brewermap(length(Rinterp), '*YlOrRd');
    ...
    subplot(2,3,1) % R
        ...
        plot(Absc, polyval(EST_R{c}{2}, Absc),'Color', colord(4,:), 'LineWidth',1.5 ); hold on
...
%         legend('5ms', '10ms', '20ms', '100ms', '500ms',  'Location', 'northwest')
        ...
        scatter(StimPair_R{c}{2}, OutPairMean_R{c}{2},25, 'x','MarkerEdgeColor', colord(4,:));hold on
...
%         legend('1', '0.9', '0.8', '0.7', '0.6', '0.5', '0.4', '0.3', '0.2', '0.1', '0', 'Location', 'BestOutside')
    ...
    subplot(2,3,2) % NDP
        ...
        plot(Absc, polyval(EST_NDP{c}{2}, Absc),'Color', colord(4,:), 'LineWidth',1.5 ); hold on
...
%         legend('5ms', '10ms', '20ms', '100ms', '500ms',  'Location', 'northwest')
        ...
        scatter(StimPair_NDP{c}{2}, OutPairMean_NDP{c}{2},25, 'x','MarkerEdgeColor', colord(4,:));hold on
...
%         legend('1', '0.9', '0.8', '0.7', '0.6', '0.5', '0.4', '0.3', '0.2', '0.1', '0', 'Location', 'BestOutside')
    ...
    subplot(2,3,3)
        ...
        plot(Absc, polyval(EST_SF{c}{2}, Absc),'Color', colord(4,:), 'LineWidth',1.5 ); hold on
        ...
        scatter(StimPair_SF{c}{2}, OutPairMean_SF{c}{2},25, 'x','MarkerEdgeColor', colord(4,:));hold on
...
b = 2; %10 ms
    subplot(1,3,1)
        plot(Absc, polyval(EST_SF{1}{b}, Absc),'Color', grey , 'LineWidth', 1.5 ); hold on
        XinOK = StimPair_SF{1}{b}(P_sf{1}{b} <= 0.05); XinNON = StimPair_SF{1}{b}(P_sf{1}{b} > 0.05);
        YmeanOK = OutPairMean_SF{1}{b}(P_sf{1}{b} <= 0.05); YmeanNON = OutPairMean_SF{1}{b}(P_sf{1}{b} > 0.05);
        YsemOK = OutPairSEM_SF{1}{b}(P_sf{1}{b} <= 0.05); YsemNON = OutPairSEM_SF{1}{b}(P_sf{1}{b} > 0.05);
        eOK = errorbar(XinOK, YmeanOK, YsemOK, 'x') ; hold on
        eOK.Color = [0 0.5 0.8];
        eNON = errorbar(XinNON, YmeanNON, YsemNON, 'x') ; hold on
        eNON.Color = [0 0 0];
    ...
    subplot(1,3,2)
        plot(Absc, polyval(EST_SF{2}{b}, Absc),'Color', grey , 'LineWidth',1.5 ); hold on
        XinOK = StimPair_SF{2}{b}(P_sf{2}{b} <= 0.05); XinNON = StimPair_SF{2}{b}(P_sf{2}{b} > 0.05);
        YmeanOK = OutPairMean_SF{2}{b}(P_sf{2}{b} <= 0.05); YmeanNON = OutPairMean_SF{2}{b}(P_sf{2}{b} > 0.05);
        YsemOK = OutPairSEM_SF{2}{b}(P_sf{2}{b} <= 0.05); YsemNON = OutPairSEM_SF{2}{b}(P_sf{2}{b} > 0.05);
        eOK = errorbar(XinOK, YmeanOK, YsemOK, 'x') ; hold on
        eOK.Color = [0 0.5 0.8];
        eNON = errorbar(XinNON, YmeanNON, YsemNON, 'x') ; hold on
        eNON.Color = [0 0 0];
    ...
    subplot(1,3,3)
        ...
        for b = 1:length(tau)
            ok = tau(b); 
            BothSF_stim{ok} = [StimPairAll_SF{1}{ok}; StimPairAll_SF{2}{ok}];
            BothSF_out{ok} = [OutPairAll_SF{1}{ok}; OutPairAll_SF{2}{ok}];
            EST_BothSF{ok} = polyfit(BothSF_stim{ok}, BothSF_out{ok},Npol);
            scatter(StimPair_SF{1}{ok}, OutPairMean_SF{1}{ok}, 'x', 'MarkerEdgeColor', okColor(b,:)); hold on % GC adult FR variation
            scatter(StimPair_SF{2}{ok}, OutPairMean_SF{2}{ok}, 'x', 'MarkerEdgeColor', okColor(b,:)); hold on % GC adult Burst variation
...
for b = 1:length(tau)
    ...
    subplot(1,length(tau),b)
        scatter(Stim_AllSpkBcap{1}{ok}, Out_AllSpkBcap{1}{ok}, 15, 'o','MarkerFaceColor', [0,0.5, 0], 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', .5); hold on
        scatter(Stim_AllSpkBcap{2}{ok}, Out_AllSpkBcap{2}{ok}, 15, '<','MarkerFaceColor', [0,0.5, 0], 'MarkerEdgeColor', 'k', 'MarkerFaceAlpha', .5); hold on
...
for b = 1:length(tau)
    ...
    subplot(1,length(tau)+1,b)
        scatter(Stim_AllSpkBinf{1}{ok}, Out_AllSpkBinf{1}{ok}, 15, 'o','MarkerFaceColor', [0,0.5, 0], 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', .5); hold on
        scatter(Stim_AllSpkBinf{2}{ok}, Out_AllSpkBinf{2}{ok}, 15, '<','MarkerFaceColor', [0,0.5, 0], 'MarkerEdgeColor', 'k', 'MarkerFaceAlpha', .5); hold on
        plot( [0 30], [0 30], 'k--'); hold on %identity line
        axis square
        box off
    ...
    subplot(1,length(tau)+1,length(tau)+1)
        scatter(Stim_AllSpkFR{1}{ok}, Out_AllSpkFR{1}{ok}, 15, 'o','MarkerFaceColor', [0,0.5, 0], 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', .5); hold on
        scatter(Stim_AllSpkFR{2}{ok}, Out_AllSpkFR{2}{ok}, 15, '<','MarkerFaceColor', [0,0.5, 0], 'MarkerEdgeColor', 'k', 'MarkerFaceAlpha', .5); hold on
        plot( [0 30], [0 30], 'k--'); hold on %identity line
        axis square
        box off
        ylabel('FRout');
...
for b = 1:length(tau)
    ok = tau(b); 
    BothBuC_stim{ok} = [StimPairAll_BuC{1}{ok}; StimPairAll_BuC{2}{ok}]; %combines datasets from input set A and B
    BothBuC_out{ok} = [OutPairAll_BuC{1}{ok}; OutPairAll_BuC{2}{ok}];
...
%          scatter(StimPair_BuC{1}{ok}, OutPairMean_BuC{1}{ok}, 'x', 'MarkerEdgeColor', okColor(b,:)); hold on % GC adult FR variation
%          scatter(StimPair_BuC{2}{ok}, OutPairMean_BuC{2}{ok}, 'x', 'MarkerEdgeColor', okColor(b,:)); hold on % GC adult Burst variation
    errorbar(StimPair_BuC{1}{ok}, OutPairMean_BuC{1}{ok}, OutPairSEM_BuC{1}{ok}, 'x', 'color', okColor(b,:)); hold on % GC adult FR variation
    errorbar(StimPair_BuC{2}{ok}, OutPairMean_BuC{2}{ok}, OutPairSEM_BuC{2}{ok}, 'x', 'color', okColor(b,:)); hold on % GC adult Burst variation
...
for b = 1:length(tau)
    ok = tau(b); 
    BothBuO_stim{ok} = [StimPairAll_BuO{1}{ok}; StimPairAll_BuO{2}{ok}]; %combines datasets from input set A and B
    BothBuO_out{ok} = [OutPairAll_BuO{1}{ok}; OutPairAll_BuO{2}{ok}];
    EST_BothBuO{ok} = polyfit(BothBuO_stim{ok}, BothBuO_out{ok},Npol);
    scatter(StimPair_BuO{1}{ok}, OutPairMean_BuO{1}{ok}, 'x', 'MarkerEdgeColor', okColor(b,:)); hold on % GC adult FR variation
    scatter(StimPair_BuO{2}{ok}, OutPairMean_BuO{2}{ok}, 'x', 'MarkerEdgeColor', okColor(b,:)); hold on % GC adult Burst variation
...
subplot(1,3,3)
    errorbar(StimPair_FR{1}{b}, OutPairMean_FR{1}{b}, OutPairSEM_FR{1}{b}, 'xk'); hold on % GC adult FR variation
    errorbar(StimPair_FR{2}{b}, OutPairMean_FR{2}{b}, OutPairSEM_FR{2}{b}, '<', 'color', [0,0.5, 0]); hold on % GC adult Burst variation
    plot(Absc2, polyval(EST_FR{1}{b}, Absc2),'Color', grey, 'LineWidth',1 ); hold on
    plot( [0 30], [0 30], 'k--'); hold on %identity line
    box off
    axis square
    xlabel('in: pairwise diff')
...
for b = 1:length(tau)
    ok = tau(b);
    CelltypesAll{b} = [Celltype{1}{ok}; Celltype{2}{ok}];
    PatSepBuCAll{b} = [PatSepBuC{1}{ok}; PatSepBuC{2}{ok}];
    ...
    subplot(1,length(tau),b)
        ...
        scatter(1.8 + 0.4.*rand(size(PatSepBuC{2}{ok})), PatSepBuC{2}{ok}, 15, 'o','MarkerFaceColor', [0,0.5, 0], 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', .5); hold on

            for ct = 1:2 %1:length(burstCT)
        okc = cet(ct);        
        ...
        set(gca,'ysc', 'lin', 'xlim',[0 3],'XTick', [1:2], 'XTickLabel', {'SH', 'EE'}); xtickangle(45)
...
for b = 1:length(tau)
    ok = tau(b); 
    PatSepBuOAll{b} = [PatSepBuO{1}{ok}; PatSepBuO{2}{ok};];
    ...
    subplot(1,length(tau)+1,b)
        ...
        scatter(1.8 + 0.4.*rand(size(PatSepBuO{2}{ok})), PatSepBuO{2}{ok}, 15, 'o','MarkerFaceColor', [0,0.5, 0], 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', .5); hold on
            for ct = 1:2 %1:length(burstCT)
        okc = cet(ct);        
    ...
    PatSepFRAll = [PatSepFR{1}{ok}; PatSepFR{2}{ok}];
    ...
    subplot(1,length(tau)+1,length(tau)+1)
        ...
        scatter(1.8 + 0.4.*rand(size(PatSepFR{2}{b})), PatSepFR{2}{b}, 15, 'o','MarkerFaceColor', [0,0.5, 0], 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', .5); hold on
        for ct = 1:2 %1:length(burstCT)
        okc = cet(ct);        
        ...
        set(gca,'ysc', 'lin', 'xlim',[0 4], 'XTick', [1:2], 'XTickLabel', {'SH','EE'}); xtickangle(45)
        title(['ANOVA1 p = ' num2str(pk2FR)]) 
        
figure(9) % Sin VS Sout: GC before gzn vs GC after gzn
clear b c cet tau
cet = 1:2; %*Collates here
tau = [2, 2, 1]; %*BinSizes here
cetCmap = [0, 0, 0; 1, 0, 0]; %*Color Map RGB
ctypes = {'SH','EE'};

% R
for b = 1:length(tau)
%ANCOVA separate lines (home-made). Ctrl: c = 1 ; Other: c = 2
    [EST_NDP_gzn{b}, pF_NDP(b), Fstat_NDP(b)] =  PolFitandFtest (Npol, StimPairAll_NDP{2}{b}, OutPairAll_NDP{2}{b}, StimPairAll_NDP{1}{b}, OutPairAll_NDP{1}{b}, 0);
    [EST_SF_gzn{b}, pF_SF(b), Fstat_SF(b)] = PolFitandFtest (Npol, StimPairAll_SF{2}{b}, OutPairAll_SF{2}{b}, StimPairAll_SF{1}{b}, OutPairAll_SF{1}{b}, 0);
    [EST_R_gzn{b}, pF_R(b), Fstat_R(b)] = PolFitandFtest (Npol, StimPairAll_R{2}{b}, OutPairAll_R{2}{b}, StimPairAll_R{1}{b}, OutPairAll_R{1}{b}, 0);

    okb = b; %tau(b)


    %NDP
    Dumout_NDP = cat(1,OutPairAll_NDP{cet}); %Lenght of cet times length of tau
    Sout_NDP{b} = cat(1, Dumout_NDP{:, okb});
    Dumin_NDP = cat(1,StimPairAll_NDP{cet});
    Sin_NDP{b} = cat(1, Dumin_NDP{:, okb});
    for c = 1:length(cet)
        okc = cet(c);
        for k = 1:length(OutPairAll_NDP{okc}{okb})
            BlockGroup_NDP{c}{k, 1} = ctypes{c};
        end
    end
    BlockGroupCat_NDP{b} = cat(1, BlockGroup_NDP{:}); clear BlockGroup_NDP
    [Hanc_NDP{b}, TABanc_NDP{b}, COEFand_NDP{b}, Sanc_NDP{b}] = aoctool(Sin_NDP{b}, Sout_NDP{b}, BlockGroupCat_NDP{b}, 0.05, 'Sin', 'Sout', 'Celltype', 'off', 'separate lines');
    COMPancS_NDP{b} = multcompare(Sanc_NDP{b}, 'Estimate', 'slope', 'display','off');
    COMPancInt_NDP{b} = multcompare(Sanc_NDP{b}, 'Estimate', 'intercept', 'display','off');
    [MAT_pvalS_NDP{b}, MAT4_pS_NDP{b} ] = pvalmat(COMPancS_NDP{b});
    [MAT_pvalI_NDP{b}, MAT4_pI_NDP{b} ] = pvalmat(COMPancInt_NDP{b});

    %SF
    Dumout_R = cat(1,OutPairAll_R{cet}); %Lenght of cet times length of tau
    Sout_R{b} = cat(1, Dumout_R{:, okb});
    Dumin_R = cat(1,StimPairAll_R{cet});
    Sin_R{b} = cat(1, Dumin_R{:, okb});
    for c = 1:length(cet)
        okc = cet(c);
        for k = 1:length(OutPairAll_R{okc}{okb})
        BlockGroup_R{c}{k, 1} = ctypes{c};
        end
    end
    BlockGroupCat_R{b} = cat(1, BlockGroup_R{:}); clear BlockGroup_R
    [Hanc_R{b}, TABanc_R{b}, COEFand_R{b}, Sanc_R{b}] = aoctool(Sin_R{b}, Sout_R{b}, BlockGroupCat_R{b}, 0.05, 'Sin', 'Sout', 'Celltype', 'off', 'separate lines');
    COMPancS_R{b} = multcompare(Sanc_R{b}, 'Estimate', 'slope', 'display','off');
    COMPancInt_R{b} = multcompare(Sanc_R{b}, 'Estimate', 'intercept', 'display','off');
    [MAT_pvalS_R{b}, MAT4_pS_R{b} ] = pvalmat(COMPancS_R{b});
    [MAT_pvalI_R{b}, MAT4_pI_R{b} ] = pvalmat(COMPancInt_R{b});
    ...
    for b = 1:length(tau)
        ...
        ylabel('pw Sout')
        set(gca, 'xlim', [0 1], 'ylim', [0 1])
        title(['R ' num2str(Xbinsize(okb)) 'ms, p = ' num2str(MAT_pvalS_R{b}), ', P(I)= ' num2str(MAT_pvalI_R{b})])
...
%NDP
    subplot(3,length(tau),b+3) 
        ...
        ylabel('pw Sout')
        set(gca, 'xlim', [0 1], 'ylim', [0 1])
        title(['NDP ' num2str(Xbinsize(okb)) 'ms, p = ' num2str(pF_NDP(b)), ', F = ' num2str(Fstat_NDP(b))])
...
% SF
    subplot(3,length(tau),b+6) 
        ...
        title(['SF ' num2str(Xbinsize(okb)) 'ms, p = ' num2str(pF_SF(b)), ', F = ' num2str(Fstat_SF(b))])
end
...
%     for b = 1:length(tau)
%         ok = tau(b);
%         [H, Pbuc89(b)] = ttest(PatSepBuC{2}{ok}-PatSepBuC{1}{ok}); clear H
%         [H, Pbuo89(b)] = ttest(PatSepBuO{2}{ok}-PatSepBuO{1}{ok}); clear H
%         [H, Pfr89(b)] = ttest(PatSepFR{2}{ok}-PatSepFR{1}{ok}); clear H
%     end
%     clear ok
%     ok = 2; % 10ms
%     subplot(1,3,1)
%       for n = 1:length(PatSepBuC{2}{ok})
%       plot(1:2, [PatSepBuC{2}{ok}(n), PatSepBuC{1}{ok}(n)], 'Color', grey); hold on
%       end
%       errorbar(1, PatSepBuC_mean{2}{ok}, PatSepBuC_sem{2}{ok}, 'ok', 'markerfacecolor', 'k', 'LineWidth', 1)
%       errorbar(2, PatSepBuC_mean{1}{ok}, PatSepBuC_sem{1}{ok}, 'or', 'markerfacecolor', 'r', 'LineWidth', 1)
%       plot( [0 4], [0 0], 'k')
%       axis square
%       grid off
%       box off
...
% 
%      subplot(1,3,2)
%       for n = 1:length(PatSepBuO{2}{ok})
%       plot(1:2, [PatSepBuO{2}{ok}(n), PatSepBuO{1}{ok}(n)], 'Color', grey); hold on
%       end
%       errorbar(1, PatSepBuO_mean{2}{ok}, PatSepBuO_sem{2}{ok}, 'ok', 'markerfacecolor', 'k', 'LineWidth', 1)
%       errorbar(2, PatSepBuO_mean{1}{ok}, PatSepBuO_sem{1}{ok}, 'or', 'markerfacecolor', 'r', 'LineWidth', 1)
%       plot( [0 4], [0 0], 'k')
%       axis square
%       grid off
%       box off
...
%     subplot(1,3,3)
%       for n = 1:length(PatSepBuC{2}{ok})
%       plot(1:2, [PatSepFR{2}{ok}(n), PatSepFR{1}{ok}(n)], 'Color', grey); hold on
%       end
%       errorbar(1, PatSepFR_mean{2}{ok}, PatSepFR_sem{2}{ok}, 'ok', 'markerfacecolor', 'k', 'LineWidth', 1)
%       errorbar(2, PatSepFR_mean{1}{ok}, PatSepFR_sem{1}{ok}, 'or', 'markerfacecolor', 'r', 'LineWidth', 1)
...
for b = 1:length(tau)
    ...
    subplot(1,length(tau)+1,b)
        ...
        box off
        ylabel('Compactness disp out - in');
        set(gca,'ysc', 'lin', 'xlim', [0 6], 'XTick', [1:2], 'XTickLabel', {'SH', 'EE'}) ; xtickangle(45);
